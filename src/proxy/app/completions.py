...
class Completions:
    ...
    def __report_exception(self, message: str, http_status_code: int) -> tuple[openai.openai_object.OpenAIObject, int]:
        """report exception"""
    ...
    def __validate_input(self, cr: CompletionsRequest):
        ...
        if cr.max_tokens and not 1 <= cr.max_tokens <= 4000:
            return self.__report_exception("Oops, max_tokens must be between 1 and 4000.", 400)

        # check the temperature is between 0 and 1
        if cr.temperature and not 0 <= cr.temperature <= 1:
            return self.__report_exception("Oops, temperature must be between 0 and 1.", 400)
        ...
        if cr.frequency_penalty and not 0 <= cr.frequency_penalty <= 1:
            return self.__report_exception("Oops, frequency_penalty must be between 0 and 1.", 400)

        # check the presence_penalty is between 0 and 1
        if cr.presence_penalty and not 0 <= cr.presence_penalty <= 1:
            return self.__report_exception("Oops, presence_penalty must be between 0 and 1.", 400)

        # check stop sequence are printable characters
        if cr.stop and not cr.stop.isprintable():
            return self.__report_exception("Oops, stop_sequence must be printable characters.", 400)
