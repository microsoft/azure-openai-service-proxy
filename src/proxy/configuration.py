...
class OpenAIConfig:
    ...
    async def __load_config(self) -> dict[str, str]:
        ...
        try:
            async with TableClient.from_connection_string(
                ...
                query_filter = f"PartitionKey eq '{self.model_class}' and Active eq true"
                ...
                if len(config) == 0:
                    ...
                    logger.warning("Found %s active OpenAI model deployments.", len(config))
    ...
    async def load_deploymemt_cache(self) -> None:
        ...
        if self.cache_expiry is None or datetime.now() > self.cache_expiry:
            ...
            self.cache_expiry = datetime.now() + timedelta(minutes=CACHE_EXPIRY_MINUTES + random.randint(0, 4))
    ...
    async def get_deployment_by_friendly_name(self, friendly_name: str) -> Deployment | None:
